...
def apply_patch(
    ...
    result = PatchResult(
        ...
        graph_hash=graph_hash(list(graph_task_keys(g)), sorted(final_edge_keys)),
        scene_hash=scene_hash(scene),
        validator_version=VALIDATOR_VERSION,
    )
    return work, result
...
def _assignment_invariant_errors(state: MissionState) -> list[ValidationError]:
    ...
    for task in state.graph.tasks:
        assigned = task.assigned_agent is not None
        if task.status in (TaskStatus.ASSIGNED, TaskStatus.RUNNING) and not assigned:
            errors.append(
                ValidationError(ErrorCode.E_SCHEMA, task.task_id, "active task has no agent")
            )
        if task.status in (TaskStatus.PENDING, TaskStatus.READY) and assigned:
            errors.append(
                ValidationError(
                    ErrorCode.E_SCHEMA, task.task_id, "released task still has an agent"
                )
            )
    return errors
