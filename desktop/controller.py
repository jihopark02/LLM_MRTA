    ...
    def _commit_next(self) -> ContinuousTick:
        ...
        for _ in range(_MAX_ZERO_LENGTH_SKIPS):
            ...
            if advance.segment is not None:
                ...
                # §22.7.1 (D-070): a queued NEW_MISSION swapped in a new graph
                # at the same sim time. The segment we just built is from the
                # old graph — drop it and re-advance so the new graph's segment
                # is played from here, no stop and no clock jump.
                if (
