...
def test_valid_mission_is_approved_and_compiled(scene):
    ...
    assert r.schema_valid and r.raw_whole_graph_valid
...
def test_workflow_error_is_repaired_then_approved(scene):
    ...
    assert not r.raw_whole_graph_valid and r.repaired_whole_graph_valid
    assert r.failure_category is None
...
def test_unknown_task_type_is_a_schema_failure(scene):
    step1 = Step1Output(tasks=[t("WATER_LOAD", "ZONE_A", 3)])
    step2 = Step2Output(edges=[])
    r = generate_mission("Load water.", scene, MockBackend([step1, step2]))
    assert not r.approved and r.attempts == 1 and not r.repaired
    assert r.failure_category == "SCHEMA" and not r.schema_valid
