...
from core.enums import Capability, TaskType
...
def test_empty_path_scores_zero(scene):
    assert path_score(ag(scene, "U1"), [], scene) == 0.0
...
def test_marginal_score_negative_inf_for_missing_capability(scene):
    # a UAV airframe stripped of SUPPRESSANT_PAYLOAD: right platform, wrong kit.
    # a UGV airframe stripped of SUPPRESSANT_APPLICATOR: right platform, wrong kit.
    inspect_only = replace(
        ag(scene, "G1"), capabilities=frozenset({Capability.GROUND_MOBILITY})
    )
    supp = task(scene, TaskType.GROUND_SUPPRESSION, "FIRE_SITE_1", 9)
    gain, _ = marginal_score(inspect_only, supp, [], scene)
