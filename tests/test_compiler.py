...
def test_ground_inspection_resolves_to_incident_access_node(scene):
    t = compile_task(scene, TaskType.GROUND_INSPECTION, "FIRE_SITE_1", priority=9)
...
def test_compile_is_deterministic(scene):
    a = compile_task(scene, TaskType.GROUND_INSPECTION, "FIRE_SITE_1", priority=9)
    b = compile_task(scene, TaskType.GROUND_INSPECTION, "FIRE_SITE_1", priority=9)
    assert a == b
...
def test_unknown_target_is_rejected(scene):
    ...
    with pytest.raises(ValueError, match="unknown incident"):
        compile_task(scene, TaskType.GROUND_INSPECTION, "FIRE_SITE_9", priority=1)
...
def test_derive_priority_incident_task_inherits_incident_priority(scene):
    assert derive_priority(scene, TaskType.GROUND_INSPECTION, "FIRE_SITE_1") == 9
...
def test_derive_priority_unknown_incident_raises(scene):
    with pytest.raises(ValueError, match="unknown incident"):
        derive_priority(scene, TaskType.GROUND_INSPECTION, "FIRE_SITE_9")


def test_reference_graph_derives_priority_from_scene(scene):
    g = compile_reference_graph(
        scene,
        [(TaskType.GROUND_INSPECTION, "FIRE_SITE_1"), (TaskType.AREA_RECON, "ZONE_C")],
        [],
    )
    assert g["GROUND_INSPECTION__FIRE_SITE_1"].priority == 9
...
def test_reference_graph_builds_and_recomputes_frontier(scene):
    ...
    assert g.ids_with_status(TaskStatus.READY) == {"GROUND_INSPECTION__FIRE_SITE_1"}
    assert g.ids_with_status(TaskStatus.PENDING) == {"GROUND_SUPPRESSION__FIRE_SITE_1"}


def test_reference_graph_rejects_unknown_edge_endpoint(scene):
    bad_edge = (
        (TaskType.GROUND_INSPECTION, "FIRE_SITE_1"),
        (TaskType.GROUND_SUPPRESSION, "FIRE_SITE_2"),  # not in the task list
    )
