...
RECHECK = (TaskType.GROUND_INSPECTION, "FIRE_SITE_1_RECHECK")
TR_F1 = (TaskType.GROUND_INSPECTION, "FIRE_SITE_2")  # a key not in the reference fixture
...
def test_clean_rewire_patch_passes_op_validation(base):
    ...
    patch = MissionPatch(
        [
            RemoveEdge(GI_F1, GS_F1),
            AddTask(*RECHECK),
            AddEdge(GI_F1, RECHECK),
            AddEdge(RECHECK, GS_F1),
        ]
    )
...
def test_add_existing_edge_conflicts(base):
    patch = MissionPatch([AddEdge(GI_F1, GS_F1)])  # already in the fixture
    assert codes(validate_patch_ops(patch, base)) == [ErrorCode.E_PATCH_CONFLICT]


def test_add_task_duplicating_existing_conflicts(base):
    patch = MissionPatch([AddTask(TaskType.GROUND_INSPECTION, "FIRE_SITE_1")])
    assert codes(validate_patch_ops(patch, base)) == [ErrorCode.E_PATCH_CONFLICT]
...
def test_edge_with_malformed_endpoint_is_schema_error(base):
    patch = MissionPatch([AddEdge(("GROUND_INSPECTION", "FIRE_SITE_1"), GS_F1)])
    assert codes(validate_patch_ops(patch, base)) == [ErrorCode.E_SCHEMA]
...
def test_post_patch_keys_is_order_independent(base):
    ...
    assert (GI_F1, GS_F1) not in a_edges
    assert (GI_F1, RECHECK) in a_edges
