...
+ `OpenAIBackend`). 테스트 196개 통과(`python3 -m pytest -q`). RQ1/RQ2 파이프라인 완성.
다음은 P6(최소 9개 입력 평가 + 시각화).

task 어휘: `GROUND_SUPPRESSION`(symbolic UGV 진압) workflow (D-016). 계약 버전 v1.18 /
최신 결정 D-019. 단계 게이트 정의는
