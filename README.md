...
**P1~P6 승인 완료** — `validator/`(P2) + `allocation/`(P3) + `execution/`(P4:
`SimExecutor`) + `llm/`(P5: Step1/Step2/repair 파이프라인 §12) + `evaluation/`(P6:
9개 입력 평가 하네스 + 감사 JSON + 시각화). 테스트 225개 통과(`python3 -m pytest -q`).
...
workflow (D-016). 계약 버전 v1.22 / 최신 결정 D-024. 단계 게이트 정의는
